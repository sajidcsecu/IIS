...
int check_eligibility_for_prescribed_heir(int heir){
    ...
    //....Sisters......//
            ...
            if(s==0 && d==0 && gs==0 && gd==0 && f==0 && b==0 && gf==0)
                return 1;
            else
                return 0;
        }
...
struct heirs* adjust_individual_shares(struct heirs *p,struct heirs *r){

       double sum=0,m,remaining=0,spouse_share=0,awal=0;
       int spouse=1;
       struct heirs *t, *l, *temp;
       t=clone(p);
       ...
       //printf("\n\tThe total distributed prescribed shares : %lf",sum);
        //printf("\n\tThe share of spouse : %lf",spouse_share);
        //printf("\n\tThe father : %d", (1-has_no_father()));
        //printf("\n\tThe mother : %d",has_mother());
        //printf("\n\tThe father : %d", (1-has_no_father()));
        //printf("\n\tThe father : %d", has_no_children());
        //printf("\n\tThe mother : %d",has_no_multiple_siblings());

       ////// umar case
        t=l;
        if(spouse_share>0 && (1-has_no_father())==1 && has_mother()==1 && has_no_children()==1 && has_no_multiple_siblings()==1 )
        {
            t=l;
            temp=go_to_node(t,5);
            temp->share=2*(sum-spouse_share)/3;
            t=l;
            temp=go_to_node(t,6);
            temp->share=(sum-spouse_share)/3;
            return l;
        }
        //else if( remaining<0 && has_grandfather()==1 && find_number_of_sisters()>=1){
          //  t=l;
            //temp=go_to_node(t,7);
            //temp->share=2*awal/3;
            //t=l;
            //temp=go_to_node(t,13);
            //temp->share=(sum-spouse_share)/3;
            //return l;
...
struct heirs* compute_shares(){
        ...
        */
        ...
        //printf("\n\t------------------------------------------------------------------------------");
        individual_earned_shares=adjust_individual_shares(list_of_prescribed_shares,list_of_residual_shares);
